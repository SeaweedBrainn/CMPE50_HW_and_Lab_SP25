...
const int DOODLEBUG = 1;	// Organism Type
const int ANT = 2;			// Organism Type
const int ANTBREED = 3;		// Number of steps to breed for Ant
const int DOODLEBREED = 8;	// Number of steps to breed for Doodlebug
const int DOODLESTARVE = 3;	// Number of steps to starve for Doodlebug
...
void World::Display()
{
	...
	cout << "\t\t\tAnt = " << antCount << ", DoodleBug = " << doodleCount;
}
...
void World::SimulateOneStep()
{
	...
	// Loop through cells in order and check if we should starve doodlebugs if
	// they have not eaten any ants in the last three steps
	for (i=0; i<WORLDSIZE; i++)
		for (j=0; j<WORLDSIZE; j++)
		{
...
class Ant : public Organism
{
 ...
 public:
	...
	virtual void breed();	// Must define this since virtual
	virtual void move();		// Must define this since virtual
	virtual int getType();	// Must define this since virtual
	virtual bool starve()   // Return false, ant never starves
	  { return false; }
};
...
void Ant::breed()
{
	...
	{
		breedTicks = 0;
		// Try to make a new ant either above, below, left, or right
		if ((y>0) && (world->getAt(x,y-1)==NULL))
		{	//above
			new Ant(world, x, y-1);
		}
...
class Doodlebug : public Organism
{
 ...
 public:
	...
	virtual void move();		// Must define this since virtual
	virtual int getType();	// Must define this since virtual
	virtual bool starve();	// Check if a doodlebug starves to death
 private:
...
void Doodlebug::move()
{
	// Look in each direction and if an ant is found move there
	// and eat it.
	if ((y>0) && (world->getAt(x,y-1)!=NULL) &&
